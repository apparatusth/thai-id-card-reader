...
export default class ThaiIDCardReader {
  ...
  init() {
    const that = this
    console.debug("ThaiSmartCardConnector init")
    const pcsc = PCSC()
    pcsc.on("reader", function (reader) {
      console.debug("New reader detected", reader.name)

      reader.on("error", function (err) {
        console.debug("Error(", this.name, "):", err.message)
      })

      reader.on("status", async function (status) {
        console.debug("Status(", this.name, "):", status)
        ...
        if (changes) {
          if (
            ...
            console.debug("card removed") /* card removed */
            ...
            console.debug("card inserted") /* card inserted */
            ...
            reader.connect(
              ...
              async function (err, protocol) {
                if (err) {
                  ...
                  console.debug("Protocol(", reader.name, "):", protocol)
                    ...
                    return new Promise((resolve, reject) => {
                      ...
                      reader.transmit(
                        ...
                        function (err, data) {
                          if (err) {
                            reader.disconnect(() => {
                              console.debug('transmit error : disconnect')
                            })
                            console.debug(err)
                          }
                          else {
                  ...
                  const getData = async (command: number[]) => {
                    ...
                    // console.debug(legacy.decode(result, "tis620"))
                  ...
                  reader.disconnect(() => {
                    console.debug('read complete disconnect')
                  })
                }
              }
            )
      ...
      reader.on("end", function () {
        console.debug("Reader", this.name, "removed")
      })
    })

    pcsc.on("error", (err) => {
      console.debug("PCSC error", err.message)
